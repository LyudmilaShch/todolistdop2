...
function App() {
    ...
    function removeTask(id: string, todolistId: number) {
        setTodo(todo.map((tl, index) => index === todolistId? {...tl, tasks: tl.tasks.filter(fl=>fl.taskId !== id)} :tl))
    ...
    function changeStatus(id: string, isDone: boolean, todolistId: number) {
        setTodo(todo.map((el, index) => index === todolistId ?  {...el, tasks: el.tasks.map(m=>m.taskId === id ? {...m, isDone}: m)} : el  ))
